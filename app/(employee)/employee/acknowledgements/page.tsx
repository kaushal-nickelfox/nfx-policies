...
const categoryIcon = (cat: string) => {
  switch (cat) {
    ...
    case 'Remote Work':
      return <CheckSquare size={16} color="#10b981" />;
    default:
...
export default function AcknowledgementsPage() {
  ...
  for (const ack of enrichedAcks) {
    const key = new Date(ack.acknowledged_at).toLocaleDateString('en-US', {
      month: 'long',
      year: 'numeric',
    });
  ...
  const pendingPolicies = (policies ?? []).filter(
    (p) => p.requires_acknowledgement && !p.is_acknowledged
  );
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {items.map((ack) => (
                    ...
                    >
                      ...
                      {/* Date + badge — hide on mobile (shown inline above) */}
                      {!isMobile && (
